...
def test_composed_derived_review_unknown_order_returns_none() -> None:
    assert (
        ProgressionService(InMemoryOrderRepository()).get_order_progression_composed_derived_review_summary(
...
def test_composed_derived_review_eligible_matches_checkpoint_and_derived_getters() -> None:
    ...
    assert cp is not None and sv is not None and rfp is not None and rf is not None and facts is not None
