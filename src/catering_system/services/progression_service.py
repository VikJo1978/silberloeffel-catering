...
class ProgressionService:
    ...
    def get_order_progression_reason_presence(
        self, order_id: str
    ) -> OrderProgressionReasonPresence | None:
        """B26: order_id + derived reason presence from B14 export only; None if order unknown."""
        ex = self.get_order_progression_export(order_id)
        if ex is None:
            return None
