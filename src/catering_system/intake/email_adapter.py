...
def _intake_from_email_body(
    ...
    if not isinstance(event_date, date):
        raise ValueError("email intake: event_date (date) is required")
    ...
    if raw.get("location_text") is None:
        ...
        location_text = "" if subj is None else str(subj)[:500]
    ...
    if guest_raw is None:
        ...
        raise TypeError("email intake: guest_count_estimate must be int or absent")
    ...
    if raw.get("planning_mode") is None:
        ...
        raise TypeError("email intake: planning_mode must be str or absent")
    ...
    if raw.get("customer_linkage") is None:
        ...
        raise TypeError("email intake: customer_linkage must be dict or absent")
    ...
    if raw.get("crm_stage") is None:
        ...
        raise TypeError("email intake: crm_stage must be str or absent")
    ...
    if raw.get("call_verification_required") is None:
        ...
        if raw.get("call_verification_status") is None:
            ...
            raise TypeError(
                "email intake: call_verification_status must be str or absent"
            )
    else:
        raise TypeError(
            "email intake: call_verification_required must be bool or absent"
        )
